...
| `MCP_CANONICAL_ORIGIN` | derived from the request `Host` header | This server's resource identifier, used to check the `aud` claim of incoming OAuth tokens. Set it to the public origin, e.g. `https://mcp.sequentum.com`. Only the origin is used — the value is normalised through `new URL(...).origin`, so a trailing slash or a path is harmless, and an `aud` naming any path on this origin is accepted. An unparseable value falls back to the caller-supplied `Host` with a warning; unset, the server warns at startup and uses `Host`. An `aud` on a different origin is logged, not rejected. |
